...
Mirrors the same stateless verification used by bookstore-tracking and
bookstore-assistant: the Django backend issues access tokens with
djangorestframework-simplejwt (HS256, signed with Django's SECRET_KEY, sent as
`Authorization: Bearer <token>` with a `user_id` claim). JWT_SECRET MUST equal
